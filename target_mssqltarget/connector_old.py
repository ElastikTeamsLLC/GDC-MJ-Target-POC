...
class mssqltargetConnector(SQLConnector):
    ...
    def create_table_with_records(
        ...
        if not full_table_name.startswith("#"):
            full_table_name = f"#{full_table_name.split('.')[-1]}"  # e.g., #test_stream

        # Use self.prepare_table, not self.connector.prepare_table
        self.prepare_table(
            full_table_name=full_table_name,
            primary_keys=primary_keys,
            schema=schema,
    ...
    #     self.connection.execute(text(droptable))
    ...
    def create_temp_table_from_table(self, from_table_name):
        ...
        tmp_full_table_name = f"#{table_name}"  # Local temp table

        droptable = f"DROP TABLE IF EXISTS {tmp_full_table_name}"
        with self._engine.connect() as conn:
            conn.execute(text(droptable))
            ddl = text(f"SELECT TOP 0 * INTO {tmp_full_table_name} FROM {full_table_name}")
            conn.execute(ddl)
            conn.commit()
